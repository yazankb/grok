...
Experiment: `multi_fixed`. Submission baseline for multi-model grokking.
...
Both contributed to the research question and final text.
...
3. Compare fine-tuning this merged model vs a random-init baseline, with the same step budget.
...
Three phases: (1) train N specialists on N disjoint shards, (2) average their weights into one model, (3) fine-tune that merged model and a random-init baseline on the full data with the same step budget.
...
| 3 | Fine-tune merged on full data (5k steps); train baseline from scratch (5k steps) | Merged and baseline metrics |
