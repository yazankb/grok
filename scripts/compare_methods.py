...
def _ema_smooth(values, alpha: float = 0.05):
    """Causal EMA that preserves NaN gaps; alpha is per-step weight on new value."""
...
def _plot_curve(ax, steps, values, label, color, *, smooth: bool = True, **kwargs):
    """Plot raw curve light + EMA-smoothed bold so noisy logs remain readable."""
    if values is None or len(values) == 0:
        return
    arr = np.asarray(values, dtype=float)
    ax.plot(steps, arr, color=color, linewidth=0.8, alpha=0.25, **kwargs)
    if smooth and len(arr) >= 5:
        ax.plot(steps, _ema_smooth(arr, alpha=0.05), color=color, linewidth=2.0, label=label, **kwargs)
    else:
        ax.plot(steps, arr, color=color, linewidth=2.0, label=label, **kwargs)
...
def plot_comparison(
    ...
    if merged_metrics.get('train_acc') is not None:
        _plot_curve(ax, merged_metrics['steps'], merged_metrics['train_acc'],
                    'Weight Avg', 'blue')
    if distill_metrics.get('student_acc') is not None and len(distill_metrics['student_acc']):
        _plot_curve(ax, distill_metrics['steps'], distill_metrics['student_acc'],
                    'Distillation', 'green')
    if baseline_metrics.get('train_acc') is not None:
        _plot_curve(ax, baseline_metrics['steps'], baseline_metrics['train_acc'],
                    'Baseline', 'gray')
    ax.set_xlabel('Step')
    ...
    if merged_metrics.get('val_acc') is not None:
        _plot_curve(ax, merged_metrics['steps'], merged_metrics['val_acc'],
                    'Weight Avg', 'blue')
    if distill_metrics.get('val_acc') is not None and len(distill_metrics['val_acc']):
        ...
        if vsteps is not None and len(vsteps) == len(val_acc):
            _plot_curve(ax, vsteps, val_acc, 'Distillation (val)', 'green')
        else:
            steps = distill_metrics.get('steps', np.array([]))
            if len(steps) >= len(val_acc):
                _plot_curve(ax, steps[: len(val_acc)], val_acc, 'Distillation', 'green')
    if baseline_metrics.get('val_acc') is not None:
        _plot_curve(ax, baseline_metrics['steps'], baseline_metrics['val_acc'],
                    'Baseline', 'gray')
    ax.set_xlabel('Step')
    ...
    if merged_metrics.get('train_loss') is not None:
        ax.semilogy(merged_metrics['steps'], merged_metrics['train_loss'],
                    label='Weight Avg', linewidth=1.0, color='blue', alpha=0.35)
        ax.semilogy(merged_metrics['steps'], _ema_smooth(merged_metrics['train_loss']),
    ...
    if distill_metrics.get('loss') is not None and len(distill_metrics['loss']):
        ax.semilogy(distill_metrics['steps'], distill_metrics['loss'],
                    label='Distillation', linewidth=1.0, color='green', alpha=0.35)
        ax.semilogy(distill_metrics['steps'], _ema_smooth(distill_metrics['loss']),
    ...
    if baseline_metrics.get('train_loss') is not None:
        ax.semilogy(baseline_metrics['steps'], baseline_metrics['train_loss'],
                    label='Baseline', linewidth=1.0, color='gray', alpha=0.35)
        ax.semilogy(baseline_metrics['steps'], _ema_smooth(baseline_metrics['train_loss']),
    ...
    if distill_metrics.get('soft_loss') is not None and len(distill_metrics['soft_loss']):
        _plot_curve(ax, distill_metrics['steps'], distill_metrics['soft_loss'],
                    'Soft Loss', 'green')
    if distill_metrics.get('hard_loss') is not None and len(distill_metrics['hard_loss']):
        _plot_curve(ax, distill_metrics['steps'], distill_metrics['hard_loss'],
                    'Hard Loss', 'orange')
    if distill_metrics.get('teacher_acc') is not None and len(distill_metrics['teacher_acc']):
        _plot_curve(ax, distill_metrics['steps'], distill_metrics['teacher_acc'],
                    'Teacher Acc', 'red', linestyle='--')
    ax.set_xlabel('Step')
...
def generate_experiment_plots(
    exp_dir: str,
    title: str = "Multi-Model Grokking: Weight Averaging vs Distillation",
    output_png: Optional[str] = None,
) -> dict:
    """
    Load metrics under exp_dir and write comparison + speedup (+ specialists) PNGs.
    Callable from training code after an experiment finishes.

    Returns a dict with keys: merged_metrics, distill_metrics, baseline_metrics,
    plot_paths (list of absolute paths to PNGs written).
    """
    exp_dir = os.path.abspath(exp_dir)
    if output_png is None:
        output_png = os.path.join(exp_dir, "comparison_plot.png")

    merged_metrics = load_metrics(os.path.join(exp_dir, "merged_average"))
    distill_metrics = load_distillation_csv_metrics(exp_dir)
    if not distill_metrics:
        distill_metrics = load_metrics(os.path.join(exp_dir, "distilled"))
    baseline_metrics = load_metrics(os.path.join(exp_dir, "baseline"))

    plot_comparison(merged_metrics, distill_metrics, baseline_metrics, output_png, title)

    speedup_path = output_png.replace(".png", "_speedup.png")
    plot_grokking_speedup(merged_metrics, distill_metrics, baseline_metrics, speedup_path)

    plot_paths = [os.path.abspath(output_png), os.path.abspath(speedup_path)]
    if os.path.isdir(os.path.join(exp_dir, "specialist_0")):
        spec_path = os.path.join(exp_dir, "specialists_curves.png")
        plot_specialists_curves(exp_dir, spec_path)
        plot_paths.append(os.path.abspath(spec_path))
...
def print_metrics_summary(
    ...
    for name, metrics in [
        ("Weight Averaging", merged_metrics),
        ("Distillation", distill_metrics),
        ("Baseline", baseline_metrics),
    ]:
...
def main():
    ...
    if args.exp_dir:
        ...
        if args.output == "comparison_plot.png":
            args.output = os.path.join(args.exp_dir, "comparison_plot.png")

    if args.exp_dir and not args.merged_dir:
        out = generate_experiment_plots(args.exp_dir, title=args.title, output_png=args.output)
        print_metrics_summary(
            out["merged_metrics"], out["distill_metrics"], out["baseline_metrics"]
        )
        return
    ...
    plot_grokking_speedup(merged_metrics, distill_metrics, baseline_metrics, speedup_path)
    print_metrics_summary(merged_metrics, distill_metrics, baseline_metrics)
