...
_LIGHTNING_2 = getattr(pl, "__version__", "0").split(".")[0].isdigit() and int(pl.__version__.split(".")[0]) >= 2
...
class TrainableTransformer(LightningModule):
    ...
    def __init__(
        ...
        :param train_dataset: optional pre-built ArithmeticDataset to use as
                              the training set, overriding prepare_data().
        :param val_dataset: optional pre-built ArithmeticDataset to use as
                            the validation set, overriding prepare_data().
        """
        super().__init__()
        self.save_hyperparameters(hparams)
        # Store custom datasets before prepare_data() so that setup() can
        # restore them after Lightning calls prepare_data() a second time
        # during Trainer.fit(), which would otherwise overwrite the shards.
        self._override_train_dataset = train_dataset
        self._override_val_dataset = val_dataset
        self.prepare_data()
    ...
    def setup(self, stage=None) -> None:
        """
        Called by Lightning after prepare_data() and before training.
        Applies any custom train/val datasets passed to __init__, which would
        otherwise have been overwritten by the second prepare_data() call that
        Lightning triggers during Trainer.fit().
        """
        if getattr(self, "_override_train_dataset", None) is not None:
            self.train_dataset = self._override_train_dataset
        if getattr(self, "_override_val_dataset", None) is not None:
            self.val_dataset = self._override_val_dataset
    ...
    def on_train_epoch_end(self):
        """
        Lightning 2.x: replaces training_epoch_end. Accumulates results of all
        forward training passes in this epoch.
        """
        ...
        if epoch_is_to_be_logged and len(outputs) > 0:
            ...
            if self.hparams.save_activations or self.hparams.save_outputs:
                if self.current_epoch == 0:
                    self._save_inputs(full_outputs, ds="train")
                self._save_activations(full_outputs, ds="train")
    ...
    def on_validation_epoch_end(self):
        """
        Lightning 2.x: replaces validation_epoch_end.
        """
...
def train(hparams: Namespace) -> None:
    ...
    # Step-based stopping: max_steps is the hard limit. max_epochs=-1 when max_steps is set
